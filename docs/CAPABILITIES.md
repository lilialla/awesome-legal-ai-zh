...
**比较口径**：比较中文法律任务覆盖、单 Skill 安装、更新机制、交付物和可验证代码，不以 Star 排序。  ·  **大类能力池**：35 项
...
**比较口径**：比较 Skill 数量、路由结构、测试框架、中国法适配和上游关系。  ·  **大类能力池**：35 项
...
**比较口径**：比较中国合同法方法、风险分层、Word 批注/修订交付、脚本和测试。  ·  **大类能力池**：32 项
...
**比较口径**：比较是否适合二次开发、方法结构是否完整、样例和评测是否可复核；不与成熟 DOCX 红线工具直接竞争。  ·  **大类能力池**：32 项
...
**比较口径**：只比较能把审查结果写回 DOCX、保留批注或修订痕迹的项目。  ·  **大类能力池**：32 项
...
**比较口径**：区分合同审查与基于固定模板、事实字段的自动填写。  ·  **大类能力池**：32 项
...
**比较口径**：比较模板执行、格式锁定、批处理、渲染验收和自动化测试。  ·  **大类能力池**：20 项
...
**比较口径**：比较事实询问、请求/抗辩结构、法源引用、法律意见书与可交付文档。  ·  **大类能力池**：20 项
...
**比较口径**：比较扫描件、表格、复杂 PDF、本地处理和后续法律切分接口。  ·  **大类能力池**：20 项
...
**比较口径**：比较是否面向法律文书批量抽取、表格/字段结构化、桌面交付、OCR 数据路径和工程测试证据。  ·  **大类能力池**：20 项
...
**比较口径**：只比较 OCR/文本提取后的条、款、项结构化切分和 MCP 接入，不与 OCR 引擎作平替比较。  ·  **大类能力池**：20 项
...
**比较口径**：比较法规数据来源、本地可用性、CLI/MCP 接口、来源元数据和回归测试。  ·  **大类能力池**：34 项
...
**比较口径**：只比较真实提供案例数据底座或可调用检索接口的项目。  ·  **大类能力池**：34 项
...
**比较口径**：比较是否在法律输出前独立核验法规版本、正文和时效。  ·  **大类能力池**：34 项
...
**比较口径**：比较材料解析、切分、检索、质量门禁、MCP/API 服务和本地数据路径。  ·  **大类能力池**：34 项
...
**比较口径**：比较官方原文优先、日期与效力状态区分、回溯去重、律师实务提示和核验失败披露。  ·  **大类能力池**：34 项
...
**比较口径**：比较文档解析、索引、混合检索、问答 API、前端工作台、外部依赖和本地测试证据。  ·  **大类能力池**：34 项
...
**比较口径**：比较案情建模、请求权/要件、证据、文书、庭审、质控和外部检索解耦。  ·  **大类能力池**：29 项
...
**比较口径**：比较请求权基础检视、要件涵摄、反论结构、法条校验和输出可复核性。  ·  **大类能力池**：29 项
...
**比较口径**：聚焦罪名发现、三阶层犯罪论、双闸确认、法条核验和 Word 交付。  ·  **大类能力池**：29 项
...
**比较口径**：聚焦可受理性、行政行为类型、合法性六要素、比例原则和裁量审查。  ·  **大类能力池**：29 项
...
**比较口径**：只比较程序日期识别、期限计算、提醒和复核记录。  ·  **大类能力池**：29 项
...
**比较口径**：比较官方 API 依赖、CLI/MCP 接口、配置诊断、批量查询和测试，不对“官方产品”宣称背书。  ·  **大类能力池**：10 项
...
**比较口径**：分别比较轻量 DOCX 脱敏和多格式本地 AI 工作台，重点看材料是否出本机、部署门槛和实测证据。  ·  **大类能力池**：21 项
...
**比较口径**：比较 APK 事实提取、隐私政策一致性、SDK/权限、法规锚定和整改清单；可运行性是当前硬门槛。  ·  **大类能力池**：21 项
...
**比较口径**：比较 DPA 专项角色判断、PIPL/GDPR/跨境隐私条款、材料完整性、签署门禁、输出边界和 benchmark 证据。  ·  **大类能力池**：21 项
...
**比较口径**：比较匿名化事实采集、可识别性判断标准、结论分级、公开来源锚定和可复核自检证据。  ·  **大类能力池**：21 项
...
**比较口径**：比较法条时效、金额计算、证据引用、仲裁文书和自动化测试。  ·  **大类能力池**：14 项
...
**比较口径**：分开处理劳动与家事，不再因劳动项目成熟就宣称家事能力已覆盖。  ·  **大类能力池**：14 项
...
**比较口径**：比较中国大陆住宅租赁预签约审查、材料隐私门禁、地方规范覆盖、结论约束和测试证据。  ·  **大类能力池**：14 项
...
**比较口径**：比较问题分解、法域识别、一手法源、引用可回溯性和面向中国律师的交付。  ·  **大类能力池**：15 项
...
**比较口径**：比较法系识别、法律效果保真、文种控制、风险术语核对、官方译文边界和交付前放行检查。  ·  **大类能力池**：15 项
...
**比较口径**：比较合同、发票、箱单、提单、产地证和中英文标签的跨文件字段核对、串单识别、境外生产企业注册校验及可复核报告。  ·  **大类能力池**：15 项
...
**比较口径**：比较名单覆盖和更新、模糊匹配、误报复核、部署门槛与面向律师的工作流。  ·  **大类能力池**：15 项
...
**比较口径**：要求至少覆盖程序管理、书状、证据、庭审或裁决书工作流，不用域外法检索项目代替。  ·  **大类能力池**：15 项
...
**比较口径**：比较收案、冲突检索、案件跟进、财务、结案归档、委托/立案材料和部署成本。  ·  **大类能力池**：20 项
...
**比较口径**：只比较中国税法研究、税务合规、争议与申报工作流，不用发票 API 或多法域税务库代表。  ·  **大类能力池**：5 项
...
**比较口径**：比较债权申报、审查、重整计划、管理人工作和清算流程。  ·  **大类能力池**：5 项
...
**比较口径**：比较不动产交易、租赁、建设工程合同、结算和索赔工作流。  ·  **大类能力池**：5 项
...
**比较口径**：只作邻近资源，不上调为中国税法实务推荐。  ·  **大类能力池**：5 项
