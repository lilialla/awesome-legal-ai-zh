...
> 快照 2026-07-22。当前推荐是编辑判断，不是客观“最好”或综合分；相似度检测只用于产生人工复核线索；工程测试通过不等于法律内容正确。
...
**比较口径**：比较中国合同法方法、风险分层、Word 批注/修订交付、脚本和测试。  ·  **大类能力池**：22 项
...
**比较口径**：比较是否适合二次开发、方法结构是否完整、样例和评测是否可复核；不与成熟 DOCX 红线工具直接竞争。  ·  **大类能力池**：22 项
...
**比较口径**：只比较能把审查结果写回 DOCX、保留批注或修订痕迹的项目。  ·  **大类能力池**：22 项
...
**比较口径**：区分合同审查与基于固定模板、事实字段的自动填写。  ·  **大类能力池**：22 项
...
**比较口径**：比较模板执行、格式锁定、批处理、渲染验收和自动化测试。  ·  **大类能力池**：12 项
...
**比较口径**：比较事实询问、请求/抗辩结构、法源引用、法律意见书与可交付文档。  ·  **大类能力池**：12 项
...
**比较口径**：比较扫描件、表格、复杂 PDF、本地处理和后续法律切分接口。  ·  **大类能力池**：12 项
...
**比较口径**：比较是否面向法律文书批量抽取、表格/字段结构化、桌面交付、OCR 数据路径和工程测试证据。  ·  **大类能力池**：12 项
...
**比较口径**：只比较 OCR/文本提取后的条、款、项结构化切分和 MCP 接入，不与 OCR 引擎作平替比较。  ·  **大类能力池**：12 项
...
**比较口径**：分别比较轻量 DOCX 脱敏和多格式本地 AI 工作台，重点看材料是否出本机、部署门槛和实测证据。  ·  **大类能力池**：15 项
...
**比较口径**：比较 APK 事实提取、隐私政策一致性、SDK/权限、法规锚定和整改清单；可运行性是当前硬门槛。  ·  **大类能力池**：15 项
...
**比较口径**：比较 DPA 专项角色判断、PIPL/GDPR/跨境隐私条款、材料完整性、签署门禁、输出边界和 benchmark 证据。  ·  **大类能力池**：15 项
