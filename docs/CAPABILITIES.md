...
**比较口径**：比较中国合同法方法、风险分层、Word 批注/修订交付、脚本和测试。  ·  **大类能力池**：30 项
...
**比较口径**：比较是否适合二次开发、方法结构是否完整、样例和评测是否可复核；不与成熟 DOCX 红线工具直接竞争。  ·  **大类能力池**：30 项
...
**比较口径**：只比较能把审查结果写回 DOCX、保留批注或修订痕迹的项目。  ·  **大类能力池**：30 项
...
**比较口径**：区分合同审查与基于固定模板、事实字段的自动填写。  ·  **大类能力池**：30 项
...
**比较口径**：比较法规数据来源、本地可用性、CLI/MCP 接口、来源元数据和回归测试。  ·  **大类能力池**：32 项
...
**比较口径**：只比较真实提供案例数据底座或可调用检索接口的项目。  ·  **大类能力池**：32 项
...
**比较口径**：比较是否在法律输出前独立核验法规版本、正文和时效。  ·  **大类能力池**：32 项
...
**比较口径**：比较材料解析、切分、检索、质量门禁、MCP/API 服务和本地数据路径。  ·  **大类能力池**：32 项
...
**比较口径**：比较官方 API 依赖、CLI/MCP 接口、配置诊断、批量查询和测试，不对“官方产品”宣称背书。  ·  **大类能力池**：9 项
...
**比较口径**：先按股权转让、融资、并购尽调拆开；首页只给已有明确中国法闸门和交割逻辑的单点项目。  ·  **大类能力池**：25 项
...
**比较口径**：比较尽调清单、材料缺口、风险分级、引证和数据室材料路径。  ·  **大类能力池**：25 项
...
**比较口径**：比较问题分解、法域识别、一手法源、引用可回溯性和面向中国律师的交付。  ·  **大类能力池**：14 项
...
**比较口径**：要求至少覆盖程序管理、书状、证据、庭审或裁决书工作流，不用域外法检索项目代替。  ·  **大类能力池**：14 项
