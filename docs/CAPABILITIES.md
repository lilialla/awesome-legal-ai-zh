...
> 快照 2026-07-27。当前推荐是编辑判断，不是客观“最好”或综合分；相似度检测只用于产生人工复核线索；工程测试通过不等于法律内容正确。
...
**比较口径**：比较中文法律任务覆盖、单 Skill 安装、更新机制、交付物和可验证代码，不以 Star 排序。  ·  **大类能力池**：32 项
...
**比较口径**：比较 Skill 数量、路由结构、测试框架、中国法适配和上游关系。  ·  **大类能力池**：32 项
...
**比较口径**：比较中国合同法方法、风险分层、Word 批注/修订交付、脚本和测试。  ·  **大类能力池**：26 项
...
**比较口径**：比较是否适合二次开发、方法结构是否完整、样例和评测是否可复核；不与成熟 DOCX 红线工具直接竞争。  ·  **大类能力池**：26 项
...
**比较口径**：只比较能把审查结果写回 DOCX、保留批注或修订痕迹的项目。  ·  **大类能力池**：26 项
...
**比较口径**：区分合同审查与基于固定模板、事实字段的自动填写。  ·  **大类能力池**：26 项
...
**比较口径**：比较模板执行、格式锁定、批处理、渲染验收和自动化测试。  ·  **大类能力池**：16 项
...
**比较口径**：比较事实询问、请求/抗辩结构、法源引用、法律意见书与可交付文档。  ·  **大类能力池**：16 项
...
**比较口径**：比较扫描件、表格、复杂 PDF、本地处理和后续法律切分接口。  ·  **大类能力池**：16 项
...
**比较口径**：比较是否面向法律文书批量抽取、表格/字段结构化、桌面交付、OCR 数据路径和工程测试证据。  ·  **大类能力池**：16 项
...
**比较口径**：只比较 OCR/文本提取后的条、款、项结构化切分和 MCP 接入，不与 OCR 引擎作平替比较。  ·  **大类能力池**：16 项
...
**比较口径**：比较法规数据来源、本地可用性、CLI/MCP 接口、来源元数据和回归测试。  ·  **大类能力池**：28 项
...
**比较口径**：只比较真实提供案例数据底座或可调用检索接口的项目。  ·  **大类能力池**：28 项
...
**比较口径**：比较是否在法律输出前独立核验法规版本、正文和时效。  ·  **大类能力池**：28 项
...
**比较口径**：比较材料解析、切分、检索、质量门禁、MCP/API 服务和本地数据路径。  ·  **大类能力池**：28 项

> **缺口**：仍缺公开的真实案卷入库、召回质量和法律问答质量基准；当前推荐不等于法律结论自动正确。

| 定位 | 项目 | 适用差异 | 上手 | 数据路径 | 依赖 | 核验 | 注意事项 |
|---|---|---|---|---|---|---|---|
| 当前推荐 | [Tsinglaw/AnyDocsMarked](https://github.com/Tsinglaw/AnyDocsMarked) | 案卷材料转 Markdown、本地 wiki/RAG 与闭世界问答形成完整链路，浅克隆语法 smoke 通过，并公开云 OCR/embedding 显式同意边界。 | 需部署 | 本地+联网 | 可选云 OCR、可选云 Embedding | Smoke 通过 | 本轮为浅克隆与语法级 smoke，未安装 anydoc 等可选依赖跑完整功能测试；项目仍处 Beta/快速迭代阶段，正式案卷入库、检索质量和法律结论必须人工复核 |
...
**比较口径**：比较文档解析、索引、混合检索、问答 API、前端工作台、外部依赖和本地测试证据。  ·  **大类能力池**：28 项
...
**比较口径**：比较案情建模、请求权/要件、证据、文书、庭审、质控和外部检索解耦。  ·  **大类能力池**：21 项
...
**比较口径**：比较请求权基础检视、要件涵摄、反论结构、法条校验和输出可复核性。  ·  **大类能力池**：21 项
...
**比较口径**：聚焦罪名发现、三阶层犯罪论、双闸确认、法条核验和 Word 交付。  ·  **大类能力池**：21 项
...
**比较口径**：聚焦可受理性、行政行为类型、合法性六要素、比例原则和裁量审查。  ·  **大类能力池**：21 项
...
**比较口径**：只比较程序日期识别、期限计算、提醒和复核记录。  ·  **大类能力池**：21 项
...
**比较口径**：先按股权转让、融资、并购尽调拆开；首页只给已有明确中国法闸门和交割逻辑的单点项目。  ·  **大类能力池**：21 项
...
**比较口径**：比较尽调清单、材料缺口、风险分级、引证和数据室材料路径。  ·  **大类能力池**：21 项
...
**比较口径**：分别比较轻量 DOCX 脱敏和多格式本地 AI 工作台，重点看材料是否出本机、部署门槛和实测证据。  ·  **大类能力池**：18 项
...
**比较口径**：比较 APK 事实提取、隐私政策一致性、SDK/权限、法规锚定和整改清单；可运行性是当前硬门槛。  ·  **大类能力池**：18 项
...
**比较口径**：比较 DPA 专项角色判断、PIPL/GDPR/跨境隐私条款、材料完整性、签署门禁、输出边界和 benchmark 证据。  ·  **大类能力池**：18 项
...
**比较口径**：比较匿名化事实采集、可识别性判断标准、结论分级、公开来源锚定和可复核自检证据。  ·  **大类能力池**：18 项
...
**比较口径**：比较法条时效、金额计算、证据引用、仲裁文书和自动化测试。  ·  **大类能力池**：10 项
...
**比较口径**：分开处理劳动与家事，不再因劳动项目成熟就宣称家事能力已覆盖。  ·  **大类能力池**：10 项
...
**比较口径**：比较中国大陆住宅租赁预签约审查、材料隐私门禁、地方规范覆盖、结论约束和测试证据。  ·  **大类能力池**：10 项
...
**比较口径**：比较技术证据提取、交底书结构、查新、本地脱敏和离线测试。  ·  **大类能力池**：10 项
...
**比较口径**：不与专利起草混合，只比较权利要求拆解、侵权比对、稳定性、FTO 和规避设计。  ·  **大类能力池**：10 项
...
**比较口径**：单独比较商标类别规划、可注册性初筛和申请材料准备。  ·  **大类能力池**：10 项
...
**比较口径**：比较源码与说明文档取样、申请表字段、材料排版和本地处理。  ·  **大类能力池**：10 项
...
**比较口径**：单独比较控制权、营业额门槛、相关市场、申报程序和风险输出。  ·  **大类能力池**：10 项
...
**比较口径**：比较 DMCA 证据留存、侵权初筛、法域识别、通知生成、链路封存和真实发送边界。  ·  **大类能力池**：10 项
...
**比较口径**：比较问题分解、法域识别、一手法源、引用可回溯性和面向中国律师的交付。  ·  **大类能力池**：11 项
...
**比较口径**：比较名单覆盖和更新、模糊匹配、误报复核、部署门槛与面向律师的工作流。  ·  **大类能力池**：11 项
...
**比较口径**：要求至少覆盖程序管理、书状、证据、庭审或裁决书工作流，不用域外法检索项目代替。  ·  **大类能力池**：11 项
...
**比较口径**：只比较个人律师的开庭/期限提醒、传票解析、卷宗看板和本地数据路径。  ·  **大类能力池**：15 项
...
**比较口径**：比较收案、冲突检索、案件跟进、财务、结案归档、委托/立案材料和部署成本。  ·  **大类能力池**：15 项
