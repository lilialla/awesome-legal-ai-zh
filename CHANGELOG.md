...
### 新增
- **近期法律实务项目复核补录**（+9 仓 → **345**）：新增 `yel66026-stack/case-material-organizer`、`christyxk-ship-it/chinese-lawyer-case-ocr-skill`、`Phoeny-Xu/ai-transparency-compliance-skill`、`f4de01/lawyer-workbench-v3`、`a1531307144-cell/LexBench`、`hwu59741-dot/offer-review-cn`、`Rethymus/LegalHigh`、`kwf18423059038-pixel/sigma-legal-bid-workflow`、`ARTHUR-BBU/Agent-T`。分别补充案件材料双向复核、本地案卷 OCR、AI 内容标识、破产节点与模板、个人法研台、录用条件比较、可追溯法律工作台、律师投标和规则/AI 分层合同审查。
- 以实务方法和独立用途为主要依据；缺许可证、测试或 CI 不单独排除，但逐项保留许可、宿主/模型外传、失败行为和核验范围。`LexBench` 因明文模型密钥随整库备份导出的风险及预置法规来源核验缺口，标为“观察”，不列关键备选。

### 能力地图
- 增加“案件材料整理与双向复核”“AI 内容透明度与标识合规”“录用条件与 Offer 比较”“律师服务投标与采购响应”四个细分任务；为“破产申报、重整与清算”补充工作台备选。均放在能力地图，首页保持 26 个任务和最多 40 个项目，不将本轮待完善工具升为首页推荐。

### 复核与使用边界
- 本轮固定公开主干提交并读取核心 Skill、实现、许可和依赖：案件材料整理 40/40 测试通过；AI 透明度 CoP digest 检查 E=0/W=0；本地 OCR 16 项中 15 通过、1 项真实 OCR 基线失败（提取文本夹杂空字符）；投标工作流 153 项中 152 通过、1 项因 PDF 工具解释器识别失败；破产出件模块 75 项中 59 通过、1 项隔离依赖失败、15 项渲染相关跳过。失败项目不标全量测试通过，未运行真实客户材料或对外提交。
- 更新 `LexBench` v0.8.5 的预置法规与可选外部 AI 描述，明确其模型调用和备份数据路径；确认破产工作台已发布 v0.2.0，第三方官方模板/指引不受其 MIT 许可覆盖。`LegalHigh` 为预发布原型；其与 AI 透明度 Skill、Agent-T 的公开可读状态不等于已授予复制、修改、分发或商用许可。
- 新增项目的 GitHub 元数据核验于 2026-09-24；未全量刷新旧项目，首页全库元数据快照日期保持原值。提交级来源链接和本轮验证结果写入 registry，并同步人工覆盖层，避免后续 audit 丢失实务判断与限制。
...
### 目录 V2 改版
- 将首页从项目平铺清单改为面向个人律师和法务的“法律 AI 选型指南”，按 20 个具体任务展示“当前推荐 / 关键备选”、能力池和比较依据；首页共可见 40 个项目，不宣称当前推荐是客观“最好”。
- 新增 [能力地图](docs/CAPABILITIES.md) 和 [完整项目索引](docs/CATALOG.md)；295 个 GitHub 仓库全部保留，辅助、相邻、观察和退役项不再与法律核心项目并列首页。
- 新增“重点作者与系列”，单列游初 / Youchu-lawhub、杨卫薪律师 / `cat-xierluo`、Tracy / `TracyWang95` 和 `CSlawyer1985` 的代表项目与审查边界，避免优质作者被单一任务分类隐藏。
- 补录 `Youchu-lawhub/cn-litigation-toolkit` 和 `Youchu-lawhub/legal-kb-builder`（+2 仓 → 295）；完整展示游初 6 仓，并将诉讼套件列为民商事诉讼全流程的当前推荐。
- 建立 V2 结构化 registry，分离人工审查、GitHub 动态元数据、任务策展、平台线索和编辑覆盖；`seed-repos.txt` 改为自动生成的兼容文件。
- 完成 295 仓可访问性和元数据核验，对 163 个法律核心或推荐候选仓执行仓库文件审阅；当前记录 13 个 `tested`、6 个 `smoke`、1 个归档仓、4 个 fork、31 个快照时超过一年未推送项。
- 对 3,070 个 `SKILL.md` 生成规范化哈希和同能力 shingle 复核线索；识别 34 组完全重复和 213 对近似候选，不据相似度自动删除项目。
- 定向实测 `cat-xierluo/contract-copilot.skill` 6 项回归通过；`TracyWang95/DataInfra-RedactionEverything` 在补齐实际依赖后 548 项通过、2 项跳过。`cat-xierluo/legal-skills` 的定向测试有 10 项通过、4 项因接口签名漂移报错；游初 APP 合规项目当前存在 `SyntaxError`，因此只列关键备选并公开风险。
- 新增 `validate`、`refresh`、`audit`、`build`、`check` 目录命令、离线 CI 和独立 GitHub 可访问性定时检查。
- 完成数据完整性、策展对抗、代码 / CI 和用户路径四路独立审查；修正制裁合规、本地法律知识库、APP 合规、法律 OCR / 文本切分、律所运营等任务的候选与缺口表述。
- 加固审计失败恢复、原子 JSON 写入、相似度稳定排序、类型校验、公开推荐可用性门禁和凭证泄露检测；明确工程测试通过不等于法律结论正确。
- 取消首页“商用”列，保留 License 和权利限制的事实提示；Star 不参与推荐排序。
...
- **涉外合同模板 / MCP 补录**（+1 仓 → **270**）：`open-agreements/open-agreements`（★39，Apache-2.0 → 商用✅）——美国/跨境合同模板与法律实践库，覆盖 NDA、SAFE/NVCA、隐私、雇佣等模板、checklists、practice guides、MCP/CLI 与 DOCX 生成；因其主要面向美国法与跨境交易模板，低位收录于涉外/跨境资源而非中国法检索或境内公司法。→ §1.5
